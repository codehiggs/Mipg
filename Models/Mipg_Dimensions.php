 ...
 */
class Mipg_Dimensions extends Model
{
    protected $table = "mipg_dimensions";
...
//$forge->addKey('profile');
    ...
    {
        $result = $this->select('dimension')->findAll();
        $sum = 0;
        foreach ($result as $r) {
            $sum += $this->get_Score($r["dimension"]);
        }
        return (round($sum / count($result), 2));
    }
     ...
     * @param type $month
     */
    public function get_Score($dimension)
    {
        $mpolitics = model("\App\Modules\Mipg\Models\Mipg_Politics", true);
        $politics = $mpolitics->get_ListByDimension($dimension);
        $count = count($politics);
        if (is_array($politics) && $count > 0) {
            $sum = 0;
            foreach ($politics as $c) {
                $score = $mpolitics->get_Score($c["politic"]);
                $sum += $score;
            }
    ...
    {
        $result = $this->where("dimension", $dimension)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
