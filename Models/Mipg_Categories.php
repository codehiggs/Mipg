 ...
 */
class Mipg_Categories extends Model
{
    protected $table = "mipg_categories";
    ...
    {
        $mactivities = model("\App\Modules\Mipg\Models\Mipg_Activities", true);
        $mscores = model("\App\Modules\Mipg\Models\Mipg_Scores", true);
        $activities = $mactivities->get_ListByCategory($category);
        $count = count($activities);
        if (is_array($activities) && $count > 0) {
            $sum = 0;
            foreach ($activities as $a) {
                $score = $mactivities->get_Score($a["activity"]);
                $sum += $score;
            }
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $component = ""): array|false
    {
        $result = $this
     ...
     * @return false|array
     */
    public function get_Category($category): false|array
    {
        $result = $this->where("category", $category)->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     */
    function get_Total(string $search = "", string $component = ""): int
    {
        $result = $this
            ->where('component', $component)
            ->groupStart()
