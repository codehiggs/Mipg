 ...
 */
class Mipg_Activities extends Model
{
    protected $table = "mipg_activities";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'order' => ['type' => 'INT', 'null' => FALSE, 'default' => '0'],
                ...
                'multiplan' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
                'budget' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $category = ""): array|false
    {
        $result = $this
            ->where("category", $category)
            ->groupStart()
            ->like("activity", "%{$search}%")
            ->orLike("category", "%{$search}%")
            ->orLike("order", "%{$search}%")
            ->orLike("criteria", "%{$search}%")
    ...
    public function get_Activity($activity): false|array
    {
        $result = $this->where("activity", $activity)->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     */
    function get_Total(string $search = "", string $category = ""): int
    {
        $result = $this
            ->where("category", $category)
            ->groupStart()
