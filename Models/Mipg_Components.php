 ...
 */
class Mipg_Components extends Model
{
    protected $table = "mipg_components";
...
//$forge->addKey('profile');
     ...
     * @param type $month
     */
    public function get_Score($component)
    {
        ...
        if (is_array($categories)) {
            ...
            foreach ($categories as $c) {
                $score = $mcategories->get_Score($c["category"]);
                $sum += $score;
            }
            if ($count > 0) {
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $diagnostic = ""): array|false
    {
        $result = $this
            ->where("diagnostic", $diagnostic)
            ->groupStart()
            ->like("component", "%{$search}%")
            ->orLike("diagnostic", "%{$search}%")
            ->orLike("order", "%{$search}%")
            ->orLike("name", "%{$search}%")
    ...
    {
        $result = $this->where("component", $component)->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     */
    function get_Total(string $search = "", string $diagnostic = ""): int
    {
        $result = $this
            ->where("diagnostic", $diagnostic)
            ->groupStart()
