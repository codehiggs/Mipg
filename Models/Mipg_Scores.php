 ...
 */
class Mipg_Scores extends Model
{
    protected $table = "mipg_scores";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'value' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'details' => ['type' => 'TEXT', 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    /**
     * Retorna el puntaje de una actividad específica.
     * @param string $activity
     * @return array|false
     */
    public function get_Score(string $activity): int
    {
        $key = $this->get_CacheKey($activity);
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where("activity", $activity)->orderBy("created_at", "DESC")->first();
            if (is_array($row)) {
                $cache = array('value' => $row['value'], 'retrieved' => true);
                cache()->save($key, $cache, $this->cache_time);
                return ($cache['value']);
            }
        }
