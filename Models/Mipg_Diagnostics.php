 ...
 */
class Mipg_Diagnostics extends Model
{
    protected $table = "mipg_diagnostics";
     ...
     * @param type $month
     */
    public function get_Score($diagnostic)
    {
        ...
        $mscores = model("\App\Modules\Disa\Mipg\Mipg_Scores");
        $components = $mcomponents->get_ListByDiagnostic($diagnostic);
        $count = count($components);
        $sum = 0;
        if (is_array($components)) {
            foreach ($components as $c) {
                $score = $mcomponents->get_Score($c["component"]);
                $sum += $score;
            }
            if ($count > 0) {
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $politic = ""): array|false
    {
        $result = $this
            ->where('politic', $politic)
            ->groupStart()
            ->like("diagnostic", "%{$search}%")
     ...
     * @return false|array
     */
    public function get_Diagnostic($diagnostic): false|array
    {
        $result = $this->where("diagnostic", $diagnostic)->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     */
    function get_Total(string $search = "", string $politic = ""): int
    {
        $result = $this
            ->where('politic', $politic)
            ->groupStart()
