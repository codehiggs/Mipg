...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Mipg\Models\Mipg_Modules");
    $mdcxm = model("App\Modules\Mipg\Models\Mipg_Clients_Modules");
    $module = 'mipg';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Mipg::index');
                $subroutes->add('/home', 'Mipg::index');
                $subroutes->add('home/(:any)', 'Mipg::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Mipg::denied');
                $subroutes->add('(:any)', 'Mipg::denied');
            }
        );
    }
}
