...
$r = $model->where('politic', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Politics.delete-message"), $name);
$back = "/mipg/politics/list/{$r["dimension"]}";
//[fields]----------------------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Politics.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Politics.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
