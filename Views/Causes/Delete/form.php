...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-08-05 15:03:44
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Plans\Views\Causes\Delete\form.php]
...
$r = $model->where('cause', $oid)->first();
$name = urldecode($r["description"]);
$message = sprintf(lang("Causes.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Causes.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Causes.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
