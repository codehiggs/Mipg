...
$r = $mdiagnostics->get_Diagnostic($oid);

$name = urldecode($r["name"]);
$message = sprintf(lang("Diagnostics.delete-message"), $name);
$back = "/mipg/diagnostics/list/{$r["politic"]}";
//[fields]----------------------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Diagnostics.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Diagnostics.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
