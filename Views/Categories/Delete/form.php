...
$r = $model->where('category', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Categories.delete-message"), $name);
$back = "/mipg/categories/list/{$r["component"]}";
//[fields]----------------------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Categories.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Categories.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
