...
$f = service("forms", array("lang" => "Plans."));
//[models]--------------------------------------------------------------------------------------------------------------
...
$r["status"] = "APPROVAL";
$r["status_approval"] = $f->get_Value("status_approval", $row["status_approval"]);
$r["status_date"] = $dates->get_Date();
$r["status_time"] = $dates->get_Time();
$r["status_approve"] = $f->get_Value("status_approve", $row["status_approve"]);
...
$statuses = array(
    ...
    array("value" => "APPROVED", "label" => "Aprobar"),
    array("value" => "REJECTED", "label" => "Rechazar"),
...
$f->fields["status_approval"] = $f->get_FieldViewArea("status_approval", array("value" => $r["status_approval"], "proportion" => "col-12"));
$f->fields["status_approve"] = $f->get_FieldTextArea("status_approve", array("value" => $r["status_approve"], "proportion" => "col-12"));
...
$f->groups["g2"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["status_approval"])));
$f->groups["g3"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["status_approve"])));
...
$card = $bootstrap->get_Card("create", array(
    "title" => "Respuesta de Aprobación del plan",
    "content" => $f,
    "header-back" => $back,
    "alert" => array(
        "type" => "info",
        "title" => lang("Plans.status-pendding-title"),
        "message" => lang("Approval.message-info"),
    ),
));
echo($card);
?>